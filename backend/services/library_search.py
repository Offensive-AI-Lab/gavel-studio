...
class HybridSearchService:
    ...
    def _fetch_candidates(self, spec: AssetSpec, bookmark_user_id: Optional[int]) -> List[dict]:
        ...
        # Visibility gating. is_ready = FALSE rows are half-generated by a
        # background pipeline (boot recovery deletes them), so they never
        # surface. Local drafts are private to the user: the public library
        # search hides them, while the bookmark-scoped variant keeps them —
        # a user's own bookmarked draft belongs on the Bookmarks page.
        where = "WHERE x.is_ready = TRUE"
        if not join_clause:
            where += " AND x.is_local_draft = FALSE"
        sql = f"""
