...
The registry is the gavel-rules GitHub repository
(Offensive-AI-Lab/gavel-rules): a CI-validated library of schema-v2
artifacts. The studio consumes exactly two kinds of files, both JSON —
**no YAML parsing**:
...
# path is always "local row exists".
#
# A public_id alone does NOT mean the record is fetchable: a local row can be
# marked published yet be absent from gavel-rules (curated out or renamed).
# Fetching those is a guaranteed 404 on every boot, so the gate is "present in
# the registry index we last synced", read from sync_state — no network call
# to find out.
...
def pull_all_aux_datasets() -> None:
    ...
    Only CEs that exist in the CURRENT registry are candidates. A database
    can hold published-looking records whose name is absent from gavel-rules
    (curated out or renamed); those are reported once here rather than
    404-ing on every boot."""
    rows = execute_query_dict(
        """
    ...
    if legacy:
        logger.info(
            ...
            "registry (legacy records absent from the current index) — "
