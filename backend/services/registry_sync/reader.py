...
The active adapter targets the gavel-rules GitHub repository. The repo is
public, so reads are anonymous; an optional GITHUB_TOKEN in backend/.env lifts
the API rate limit (and authenticates a private GAVEL_RULES_REPO override).
This adapter READS ONLY — the studio never writes to the repo (contributions
go by pull request, outside the studio).
"""
...
def build_reader() -> RegistryReader:
    ...
    Env (all optional — the default repo is public):
      GAVEL_RULES_REPO  owner/name of the registry repo
      GAVEL_RULES_REF   branch/tag/SHA to sync from (default main)
      GITHUB_TOKEN      lifts the anonymous API rate limit (60/h -> 5000/h);
                        required only for a private GAVEL_RULES_REPO
    """
