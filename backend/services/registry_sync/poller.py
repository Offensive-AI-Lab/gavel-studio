"""Registry update poller — periodic freshness probe against the library repo.

The backend checks the gavel-rules repository on a timer. Each tick runs a
reconcile action (see wiring._LibraryUpdateNotifier): a cheap manifest-hash
compare against `sync_state.last_manifest_hash` — no records are pulled, so a
tick costs one small read. The result drives `library_events.set_available`
...
`client.reconcile()` does blocking network I/O, so it's dispatched to a thread.
"""
...
class RegistryUpdatePoller:
    ...
    async def _reconcile(self) -> Optional[dict]:
        # reconcile() is blocking (network I/O + DB) → run off the event loop. A
