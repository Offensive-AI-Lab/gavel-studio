...
# (CORS origins) overrides the file's native defaults.
...
async def lifespan(app: FastAPI):
    ...
    # probe against the library repo that drives the sidebar's "Updates
    # available" badge. Non-fatal if it can't start. ---
    poller = None
    try:
...
# Pre-fetch the public library on a background thread so the first page load
# doesn't pay the full registry round-trip. Sync is idempotent + manifest_hash
# short-circuits, so later sync calls degrade to a near no-op once this
# lands. Spawned only after init_database() ran (the _warmup_state["db"]
# flip above) — it must not start until the tables are confirmed ready,
# since sync_library writes into rules / cognitive_elements / categories.
...
# up only its own leaks from a prior backend that died mid-run. Daemon thread,
# best-effort.
