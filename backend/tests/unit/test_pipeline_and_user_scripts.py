...
network. We assert on:
  ...
  * user lookup/create upsert helpers and the legacy fallback in
...
class TestEnsureCreatorsInLocal:
    """After a library sync, every `created_by_username` on the imported
    rules/CEs must exist locally so attribution JOINs and profile lookups
    resolve. There is no central user directory, so unknown authors get a
