...
def temp_db(monkeypatch, tmp_path):
    """Point the sqlite layer at a fresh temp file and build the v24 schema."""
...
class TestColdSync:
    ...
    def test_result_dict_has_no_neutral_key(self, reader):
        # neutral_synced died with the HF registry (no upstream source).
...
class TestValidation:
    ...
    def test_unknown_member_ce_errors(self, reader):
        entry = reader.index["rules"]["electoral_political_content_creation"]
        entry["groups"] = {"g": ["content_creation", "ghost_ce"]}
        entry["condition"] = "all of g"
        result = ls.sync_library()
...
# ===========================================================================
# Legacy records: a public_id from the RETIRED HuggingFace library does not
# mean the record exists in gavel-rules. Those must never be fetched (every
# attempt is a guaranteed 404 on every boot).
# ===========================================================================


class TestLegacyRecordsAreNotFetched:
    @staticmethod
    def _seed_legacy_ce(name="self_harm", public_id="ce_legacy_hf"):
        """A row shaped like one carried over from the HF-era library:
        published (public_id set, not a draft) but absent from the registry."""
        execute_query(
