...
_TRACKED_TABLES = [
    ...
    # COMPOSITE keys — see note above. Membership-only: rule logic lives in
    # the parent rows' ce_groups/condition columns, not on the links.
    ("setup_ce_link", ("setup_id", "ce_id")),
    ("rule_ce_link", ("rule_id", "ce_id")),
    # Bookmarks are plain local tables (there is no login), so they need
    # per-test cleanup. Ratings are gone entirely.
