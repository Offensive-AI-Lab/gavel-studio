...
class TestLibrarySearchExtras:
    ...
    def test_categories_endpoint_returns_list(self, client, auth_headers):
        ...
        assert res.status_code == 200
        data = res.json()
        # Categories come from the registry sync (categories.json) rather than
        # a hardcoded local seed; on a fresh DB pre-sync the list may be empty.
...
class TestNormalizeCategories:
    def test_normalize_existing_categories_by_name(self):
        ...
        # Categories are not auto-seeded at DB init (the registry sync is the
