...
class TestRunInferenceDispatch:
    """_run_inference offloads to the remote GPU worker when configured, and
    falls back to local GPU/CPU inference when the worker is off OR errors —
    never failing the run just because the worker is unavailable."""
    ...
    def test_uses_local_when_no_worker_configured(self, tmp_path, monkeypatch):
        # With no worker configured, the failover chain contains only the local
        # tiers — the worker transport is never touched.
        ...
        # GPU_PROVIDER / GPU_WORKER_URL are read at import time into module
        # constants, so a developer's backend/.env would otherwise decide this
        # test's outcome. Pin auto-mode + no worker explicitly.
        monkeypatch.setattr(compute_registry, "_PROVIDER", "auto")
        monkeypatch.setattr(compute_registry, "_WORKER_URL", "")
        ...
        try:
            with patch("classifier_engine.trainer.classifier_workdir", return_value=str(tmp_path)), \
                 patch("services.compute.providers.remote_worker.RemoteWorkerProvider.run_inference") as worker, \
    ...
    def test_falls_back_to_local_when_worker_raises(self, tmp_path, monkeypatch):
        import evaluation.inference  # ensure submodule exists for patching
        from unittest.mock import patch
        from routes import evaluation as ev
        ...
        from services.compute import registry as compute_registry
        monkeypatch.setattr(compute_registry, "_PROVIDER", "auto")
        monkeypatch.setattr(compute_registry, "_WORKER_URL", "http://localhost:1")
        compute_registry.invalidate_cache()
        self._write_artifacts(tmp_path)
        try:
            with patch("services.compute.providers.remote_worker.RemoteWorkerProvider.is_available",
                       return_value=True), \
                 patch("classifier_engine.trainer.classifier_workdir", return_value=str(tmp_path)), \
                 patch("services.compute.providers.remote_worker.RemoteWorkerProvider.run_inference",
                       side_effect=compute.ComputeError("worker boom", retryable_local=True)) as worker, \
                 patch("evaluation.inference.run_inference_on_dialogues",
    ...
    def test_uses_worker_when_enabled(self, tmp_path, monkeypatch):
        import evaluation.inference  # ensure submodule exists for patching
        from unittest.mock import patch
        from routes import evaluation as ev
        from services.compute import registry as compute_registry
        ...
        try:
            with patch("services.compute.providers.remote_worker.RemoteWorkerProvider.is_available",
                 ...
                 patch("classifier_engine.trainer.classifier_workdir", return_value=str(tmp_path)), \
                 patch("services.compute.providers.remote_worker.RemoteWorkerProvider.run_inference",
                       return_value=[{"logits": "from_worker"}]) as worker, \
                 patch("evaluation.inference.run_inference_on_dialogues") as local:
                out = ev._run_inference(9, [{"conversation": [], "metadata": {}}])
        finally:
            # Don't leak the fake worker availability into later tests.
            compute_registry.invalidate_cache()
        assert out == [{"logits": "from_worker"}]
        worker.assert_called_once()
...
class TestDefaultEvalPairsPerRule:
    ...
    def test_pairs_carry_each_rules_own_name(self, client, test_model, test_user, auth_headers):
        ...
        # user_id is NOT NULL on classifiers. Supply the owner explicitly so
        # the test matches the schema (and every real code path).
