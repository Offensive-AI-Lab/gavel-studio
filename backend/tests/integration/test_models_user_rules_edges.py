...
(HuggingFace validation) or the earlier multi-user server (register/login/me) use
...
class TestUserMeAuthBoundaries:
    """/user/me has no token handling — there is no login."""

    def test_me_without_token_is_allowed(self, client):
        """No Authorization header → the request still succeeds (no auth)."""
