...
class TestAuxTablesExist:
    ...
    def test_rule_evaluation_table_dropped(self, client):
        # Guards against reintroducing a `rule_evaluation_datasets` table:
        # test sets are rule-scoped rows in test_datasets, and there should
        # be no trace of a separate table after init_database.
        from utils.sqlite_db import execute_query_dict
        rows = execute_query_dict(
            """
            SELECT 1 FROM sqlite_master
