...
There is no login and no central user directory. The `users` table now holds:
  ...
  * placeholder rows for OTHER authors whose rules/CEs arrive from the shared
    gavel-rules library, created by `ensure_creators_in_local()` so
...
and email are unknown for a remote author. They
...
def ensure_creators_in_local(usernames: list):
    """After a library sync imports rules/CEs, make sure every `created_by_username`
    ...
    for name in missing:
        try:
            ...
            execute_query(
                """
                INSERT INTO users (user_id, username, email, is_team)
