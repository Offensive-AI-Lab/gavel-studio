...
def commit_trained_policy_snapshot(classifier_id: int) -> None:
    ...
    Call ONLY on SUCCESSFUL training completion — local (trainer.py) AND remote
    GPU (the get-status completion path). Writing it any earlier (e.g. at
    training start) makes an INTERRUPTED run look 'Up to Date' on a model that
    was never produced; never recording it leaves the drift banner stuck after
    ...
    # Leave any '*_running' markers alone — they may own a live compute job and
...
def resolve_editor_logic(groups: dict, condition: str):
    ...
    condition) into the stored groups+condition shape.
...
def get_classifier_rules(classifier_id: int):
    ...
    Each row carries:
      * `logic` — {"groups": {group_name: [{ce_id, name}]}, "condition",
        ...
        (the groups+condition contract the rule editor speaks).
...
def fork_setup_to_draft(
    ...
    condition-grammar string over those group names.
