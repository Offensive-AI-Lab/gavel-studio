...
def load_reference_examples():
    ...
    excitation row. After the lazy-excitation optimization, registry-synced
    CEs only get their excitations on-demand — which starves this reference
    pool and makes new-CE generation much worse for the LLM (no examples
    to mimic). To compensate, before reading the join we ask the library-sync
    ...
    isn't slow.
    """
    # Best-effort prefetch — never block the cache build on registry problems.
    try:
