...
named CE groups + a condition string in the gavel-rules condition grammar
(utils/rule_condition); the display predicate is derived from them via
render_predicate. Every defined group must be referenced by the condition —
unreferenced ("supporting") groups are rejected, matching the library CI.
"""
import json
import re
...
def validate_rule(rule_data, ces_dict):
    """Validates the LLM rule output: groups/condition shape, the condition
    ...
    # Validate the condition against the groups. validate_condition mirrors
    # the library CI (grammar, group refs, dead groups, quantifier bounds,
    # negation rejected), so studio rules stay publishable to gavel-rules.
