...
# Per-guardrail lock for remote-artifact downloads. Status polling fires every 5s
...
def _cancel_remote_inference_for_classifier(classifier_id: int) -> None:
    """If the guardrail has an in-flight remote calibration/evaluation job,
    cancel it and clean its remote job dir BEFORE the DB delete. The job's
    pointer is stashed on the '*_running' evaluation_results row's `plots`
    column (see routes/evaluation.py _run_inference._stash); the cascade
    delete will wipe that row, so we must read it first. Best-effort —
        ...
        for row in rows:
            ...
            if isinstance(plots, str):
                try:
                    ...
                    continue
            pointer = (plots or {}).get("remote_job") if isinstance(plots, dict) else None
...
def _end_realtime_session_for_classifier(classifier_id: int) -> None:
    """Tear down any warm remote realtime session for this guardrail so
    deleting a guardrail mid-monitoring doesn't leave a GPU job running.
    Best-effort."""
    try:
        from services import compute
...
def start_training(
    ...
    # Lazy-load any registry-synced CE excitations that aren't cached locally yet.
    try:
...
def get_training_status(classifier_id: int, auth_uid: int = Depends(get_current_user)):
    ...
    if _job_info:
        ...
        def _ret(status, *, model_path=None, phase=None, detail=None, training_log=None,
            ...
            # to be reported on this path too — otherwise a remotely-trained rule
        ...
        def _failover(reason: str):
            ...
            crashed/vanished worker re-runs the training elsewhere
        ...
        try:
            ...
            # Running - liveness deadline fails a job whose worker vanished.
