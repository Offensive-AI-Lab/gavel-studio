...
#   {"groups": {group_name: [ce_id, ...]}, "condition": "<v2 grammar>"}
...
class UpdateLogicRequest(BaseModel):
    ...
    condition: str = Field(..., max_length=4000)  # v2 grammar over group names
...
def get_rule_detail(rule_id: int, _: int = Depends(get_current_user)):
    ...
    in no group, which shouldn't happen post-v24)."""
...
def create_public_rule(req: CreatePublicRuleRequest, _: int = Depends(get_current_user)):
    """Create a rule from named CE groups + a v2 condition.
