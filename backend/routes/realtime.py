...
def _friendly_local_error(e: Exception) -> str:
    ...
    that clearly (with the fix: use a GPU worker) instead of a raw torch trace."""
    s = str(e).lower()
    mem_markers = (
...
def _score_span(ctx: dict, windows: list, tokens: list):
    ...
    # --- Rule conditions (groups + condition grammar, evaluated over triggers) ---
    ...
    for rule_name, spec in ctx["unified_ruleset"].items():
        ...
        for gname, members in (spec.get("groups") or {}).items():
            ...
            if quant is None:
                # Unreferenced group on a legacy row — validation now rejects
                # these at save time (no dead groups), but stored rows from
                # before the rule may still carry one; it never gates firing.
                satisfied = True
            else:
...
# ---------------------------------------------------------------------------
# Warm remote realtime SESSION — the LLM forward runs on the remote GPU so
...
# A session here runs OFF this machine — on the remote GPU worker (HTTP), through
...
def _active_session(classifier_id: int):
    """(provider, session) for a warm realtime session on the remote worker, or
    None when there is no off-box session (local mode)."""
    return _session(classifier_id)
...
def session_start(classifier_id: int, _: int = Depends(get_current_user)):
    """Submit a warm realtime job on the remote worker. Returns immediately;
    poll /session/status until it reports 'ready' (the model is loading).

    Returns {fallback:'local'} (NOT an error) when the warm session can't be
    used — no worker is configured, or the guardrail uses a LOCALLY-uploaded
    model that exists only on this machine's disk (the worker can't load it). The
    ...
    # (HTTP). Local mode has no warm session (the client runs the model
    # in-process), so anything else returns {fallback:'local'}. A user-uploaded
    # (local-filesystem) model can't be loaded off-box either. The provider
    # resolution already accounts for reachability (an unreachable worker
    # resolves to 'local' here), so this route stays transport-agnostic.
    from services import compute
    # The viewer re-calls this endpoint on crash detection, so this also drives
    # mid-session recovery (worker -> local in-process mode).
...
def session_end(classifier_id: int, _: int = Depends(get_current_user)):
    """Tear down the session (stop sentinel + job cancel + cleanup). Called on every
...
def session_analyze_stored(classifier_id: int, req: AnalyzeStoredRequest, _: int = Depends(get_current_user)):
    ...
    on the remote GPU; the backend applies thresholds/rules. Same shape as
...
def session_analyze(classifier_id: int, req: AnalyzeRequest, _: int = Depends(get_current_user)):
    ...
    it on the remote GPU; the backend applies thresholds/rules."""
    _require_trained_classifier(classifier_id)
    meta, _rnn = _resolve_for_session(classifier_id)
    labels = meta["labels"]
    s = _active_session(classifier_id)
