...
def _repair_rule_json(raw_response: str, ces_dict: Dict[str, Dict]):
    ...
    system_msg = {
        ...
        "content": (
            ...
            "every group defined in 'groups' must be referenced by the condition), "
...
async def get_ce_training_dataset(ce_id: int):
    ...
    try:
        # Lazy-load the excitation from the registry if this is a synced CE
        # whose dataset wasn't pulled at sync time. No-op when the row is
        # already cached locally; one registry round-trip on a miss.
        try:
...
# (published library rules ship theirs in the registry). Two creation paths
# converge on `generate_rule_defaults`:
...
def get_rule_defaults(rule_id: int, _: int = Depends(get_current_user)):
    ...
    calibration/evaluation steps without generating anything. Lazily pulls the
    sets from the registry first if the rule is public and they aren't local yet.
    """
    try:
...
def preview_rule_test_sets(rule_id: int, user_id: int = Depends(get_current_user)):
    ...
    Lazily pulls the default from the registry first if the rule is public.
    """
    try:
        from services.library_sync import ensure_rule_defaults
        ensure_rule_defaults(rule_id)
...
# The two paragraph-long instruction blocks the reference prompt injects
# directly into the dialogue generator — without them the LLM treats
# `interaction_mode: conversational` as just a label and keeps producing
# AI-assistant-sounding text.
