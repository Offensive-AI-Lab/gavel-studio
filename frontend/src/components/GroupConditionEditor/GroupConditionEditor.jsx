...
//   * a per-group quantifier picker (all / at least k)
...
import {
    ALL, toEditorModel, buildCondition, clampQuant, describeCondition,
...
// strings and we need to distinguish "all" from a member count.
const quantValue = (quant) => (quant === ALL || quant === undefined ? ALL : String(quant));
...
export default function GroupConditionEditor({ pool, groupList, condition, onChange }) {
    ...
    const usedCount = (groupList || []).filter((g) => (g?.name || '').trim()).length;
            ...
            <div>
                ...
                {representable ? (
                    <p style={{ margin: '6px 0 0', fontSize: '0.75rem', color: '#64748b' }}>
                        Written for you from the choices above. {describeCondition(groupList, joiner)}
                        {' '}Every group is part of it — delete a group you do not need.
                    </p>
                ) : (
