...
// Every group is part of the condition: either required in full, or required
// to match at least k of its members. A group the condition never mentions is
// rejected by the backend (utils/rule_condition) and by the library's CI, so
 ...
 * the text tracks what the user sees on screen. Every named group gets a
 * selector — a group with no quantifier yet defaults to all-of rather than
 * being left out of the condition.
 */
export function buildCondition(groupList, joiner, quants) {
    ...
    (groupList || []).forEach((g) => {
        ...
        parts.push(`${quant === undefined || quant === ALL ? ALL : quant} of ${name}`);
    });
 ...
 *
 * A group the stored condition does not mention (a legacy rule from before
 * every group had to be referenced) comes back without a quantifier and is
 * treated as all-of, so the next edit writes it into the condition.
 */
 ...
 * group can never fire, and "at least 2" in a two-member group has to be
 * written "all of" — the library's validator rejects both.
 */
export function clampQuant(quant, memberCount) {
    if (quant === ALL || quant === undefined) return quant;
 ...
 */
export function describeCondition(groupList, joiner) {
    const used = (groupList || []).filter((g) => (g?.name || '').trim());
