// Rule-logic helpers for the groups + condition model (schema v2).
//
// A rule's firing logic is now:
//   * `groups`    — named CE groups: { group_name: [members] } where a member
//                   is either a CE name (string) or { ce_id, name }.
//   * `condition` — a v2 grammar expression over group names, e.g.
...
// this only catches the obvious mistakes before a round-trip.
