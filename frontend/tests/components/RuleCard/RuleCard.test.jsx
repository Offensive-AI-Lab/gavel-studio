...
//     bookmark / export / rule-page / delete buttons, chevron)
...
describe('RuleCard — expanded body, read view', () => {
    it('falls back to the predicate string when the rule has no groups', () => {
        ...
        expect(screen.getByText('Firing Logic')).toBeInTheDocument();
        expect(screen.getByText('A AND B')).toBeInTheDocument();
    });

    it('renders group chips titled by group name plus the condition', () => {
        render(<MemoryRouter><RuleCard rule={groupedRule()} isExpanded onToggle={() => {}} /></MemoryRouter>);
        // Group name headers — each name appears twice: on the group pill and
        // color-coded inside the condition text.
        ...
        // Condition rendered in full (group names are tinted spans within it)
