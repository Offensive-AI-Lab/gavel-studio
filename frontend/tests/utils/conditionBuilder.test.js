...
import {
    ALL, parseCondition, buildCondition, toEditorModel, clampQuant,
    describeCondition,
...
describe('buildCondition', () => {
    it('defaults a group with no quantifier to all-of', () => {
        // The backend rejects a group the condition never names, so there is no
        // way to leave one out.
        expect(buildCondition(groups('a', 'b'), 'and', {})).toBe('all of a and all of b');
    });
    ...
    it('references every group in the list', () => {
        expect(buildCondition(groups('a', 'b', 'c'), 'and', { a: ALL, c: 1 }))
            .toBe('all of a and all of b and 1 of c');
    });
...
describe('toEditorModel', () => {
    it('reads an empty condition as no quantifier chosen yet', () => {
    ...
    it('writes a legacy unreferenced group back into the condition', () => {
        // Rules stored before every group had to be referenced still open in
        // the pickers; the rebuilt condition names the missing group.
        const model = toEditorModel(groups('a', 'b'), 'all of a');
        expect(model.representable).toBe(true);
        expect(model.quants.b).toBeUndefined();
        expect(buildCondition(groups('a', 'b'), model.joiner, model.quants))
            .toBe('all of a and all of b');
    });
...
describe('clampQuant', () => {
    it('leaves all-of alone', () => {
        expect(clampQuant(ALL, 3)).toBe(ALL);
        expect(clampQuant(undefined, 3)).toBe(undefined);
    });
    ...
    it('falls back to all-of when k equals the group size', () => {
        // The validator rejects "2 of a 2-member group" — that is "all of".
        expect(clampQuant(2, 2)).toBe(ALL);
        expect(clampQuant(1, 1)).toBe(ALL);
    });
...
describe('describeCondition', () => {
    it('warns when nothing would ever fire', () => {
        expect(describeCondition([], 'and')).toMatch(/never fires/i);
    });

    it('describes and / or in words', () => {
        expect(describeCondition(groups('a', 'b'), 'and')).toMatch(/ALL/);
        expect(describeCondition(groups('a', 'b'), 'or')).toMatch(/ANY/);
    });

    it('does not talk about combining when there is only one group', () => {
        expect(describeCondition(groups('a'), 'and')).toMatch(/this group/i);
    });
});
